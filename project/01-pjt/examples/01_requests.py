...
# 오늘 수 업에서는 jsonplaceholder라고하는 sample형 json 데이터 제공 api를 사용할 것
...
print(user_response)
print(response)
compledted_todos = [] # 최종 결과물을 담을 리스트
fields = ['id','title']
...
for item in response:
    ...
    if item.get('completed'):
        ...
        for key in fields: # id, title
            ...
            # {'id': 1, 'title': loremipsu,...}
        compledted_todos.append(temp_item)
        for user in user_response:
            if user['id'] == item['userId']:
                user_info = {
                    # ctrl + alt +  위아래방향키 : 커서 복제
                    # ctrl + shift + 좌우방향키 : 단어 단위로 드래그
                    'id':user['id'],
                    'name':user['name'],
