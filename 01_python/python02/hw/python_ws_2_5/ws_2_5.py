...
# 아래에 코드를 작성하시오.
class Theater:
  def __init__(self, name, total_seats,reserved_seats=0):
    self.name = name
...
# 자식클래스에서 init을 정의하지 않으면 부모클래스의 init을 사용한다.
class MovieTheater(Theater):
  ...
  @staticmethod
  def description(theater_instance):
    print(f"영화관 이름: {theater_instance.name}")
    print(f"총 좌석 수: {theater_instance.total_seats}")
    print(f"예약된 좌석 수: {theater_instance.reserved_seats}")
