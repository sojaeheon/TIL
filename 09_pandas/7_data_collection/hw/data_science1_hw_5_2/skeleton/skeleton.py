...
BUS_API_KEY = "4566655379736f6a35376f596f7062"  # 서울시 버스 교통 데이터 API 키 (빈칸 채우기)
SUBWAY_API_KEY = "6e68637062736f6a33324159584c51"  # 서울시 지하철 교통 데이터 API 키 (빈칸 채우기)
...
def get_subway_data(date):
    ...
    url = f"{SUBWAY_URL}{SUBWAY_API_KEY}/xml/CardSubwayStatsNew/1/5/{date}"  # SUBWAY_URL과 SUBWAY_API_KEY, date를 이용하여 API 요청 URL 생성)
...
def integrate_data(bus_data, subway_data):
    ...
    subway_data = bus_data.rename(columns={'정류장명': '역/정류장명'})  # subway_data에서 '역명' 컬럼을 '역/정류장명'으로 변경)
    ...
    return integrated_df[['교통수단', '날짜', '노선번호', '역/정류장명', '승차인원', '하차인원']]  # 최종적으로 '교통수단', '날짜', '노선번호', '역/정류장명', '승차인원', '하차인원' 컬럼만 포함한 데이터프레임 반환)  # 최종적으로 '교통수단', '날짜', '노선번호', '역/정류장명', '승차인원', '하차인원' 컬럼만 포함한 데이터프레임 반환)
